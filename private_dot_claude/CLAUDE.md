...
**コード関連作業の既定の委譲先は T3 実行サブエージェント**。Codex（T2）は Codex 優位領域 / 難所 / 大規模実装の 3 条件に当てはまるときだけ選ぶ。Codex は週間枠が有限で、Codex Cloud の PR レビューと同じ枠を食い合うため。

委譲マトリクスの実体（司令塔のモデル、実行役が Codex か実行サブエージェントか）は環境によって変わる。着手前必須チェック・役割ベースの Tier 定義・レート予算の規律（委譲サイズと回数の上限）・Codex 未導入時のフォールバック分岐・Codex 呼び出しテンプレート・アンチパターンはすべて `~/.claude/skills/task-delegation/SKILL.md`（委譲体系の正本）を参照。
