...
- **報告・回答は「概要がすぐ伝わるもの」を先頭に置く**。まず結論を1行、続けて表 / 箇条書き / サマリで全体像を提示し、**そのうえで補足が必要なものだけを後段で説明する**。長い散文の理由・前提を冒頭にだらだら書かない。表で済む比較は表にする（例: 複数 PR/Issue の状態・順序は一覧表 → 補足で「なぜその順か」）。`[#123](URL)` リンク併記の対象は後述の Issue/PR ルールに従う
...
- **番号・記号だけで人間に言及しない（最重要・繰り返し指摘されている）**。GitHub Issue/PR 番号・手順書やドキュメントの Step 番号・章節番号・略号などを、番号/記号だけで出さない。必ず「それが何を指すか」を言葉で添える。AI は番号で整理しやすくても、人間にとって番号は索引にすぎず中身を想起できない。報告・サマリ・テーブル・通常テキスト・`AskUserQuestion` の質問文すべてに適用する。
  - ❌「#320 を見て」「Step 8.5 の話です」「8.2.3 が件数を出す」
  - ✅「#320 SpecDD 移行 Epic」「リスク一覧を作る処理（手順書の Step 8.5）」「子タスクの取りこぼしチェック（Step 8.2.3）は件数を出す」
  - まず日常語で「何の処理／何の Issue か」を言い、番号は補足として括弧で添える。内部の通し番号それ自体に意味はないと心得る
...
委譲マトリクスの実体（司令塔が Opus か Fable か、実行役が Codex か impl-sonnet / impl-opus か）は config dir で変わる。着手前必須チェック・役割ベースの Tier 定義・config dir 分岐・Codex 呼び出しテンプレート・アンチパターンはすべて `~/.claude/skills/task-delegation/SKILL.md`（委譲体系の正本）を参照。
