...
対象 PR が明確で観点評価だけ即時に欲しい単発の場合は `/peer-review` を直接起動してもよい。複数 PR のトリアージや投稿判断の対話が要る場合は `/pr-reviewer` を使う（詳細な役割の違いは上表を参照）。
...
`/pr-reviewer`（内部で `/peer-review` を使う）で PR 全体を俯瞰しつつ、ADR ファイルは個別に `/review-adr` で深掘りする 2 段構成。

優先順位:
1. `/pr-reviewer` で PR 全体（コード + ADR + ドキュメント）の俯瞰指摘を収集
2. ADR について論理一貫性・既存矛盾を深掘りしたい場合のみ `/review-adr` を追加実行
3. 重複指摘は統合し、PR コメント投稿は `/pr-reviewer` の投稿承認フローに乗せて 1 回に集約する
