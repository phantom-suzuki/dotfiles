...
`/peer-review` と `/pr-reviewer` の使い分けは対象が単発か複数 PR のトリアージかで決まる。判定基準は次の判定フロー Q2 を参照（詳細な役割の違いは上表）。
...
Q2: 対象は誰のもの？
  ├─ 他者の PR
  │   ├─ 対象が明確で観点評価だけ即時に欲しい（単発） → /peer-review
  │   └─ レビューリクエスト一覧・トリアージ・投稿判断の対話が要る → /pr-reviewer
  └─ 自分のもの → Q3 へ
...
PR 全体（コード + ADR + ドキュメント）を俯瞰しつつ、ADR ファイルは個別に `/review-adr` で深掘りする 2 段構成。俯瞰側の起動先は Q2 と同じ基準で選ぶ（単発で対象が明確なら `/peer-review`、複数 PR のトリアージ・投稿判断の対話が要るなら `/pr-reviewer`）。

優先順位:
1. Q2 の基準で選んだスキルで PR 全体の俯瞰指摘を収集
2. ADR について論理一貫性・既存矛盾を深掘りしたい場合のみ `/review-adr` を追加実行
3. 重複指摘は統合し、投稿は手順 1 で選んだスキルのフローに乗せて 1 回に集約する
