...
description: 現セッションの到達点を memory に保存し、次セッション用の起動プロンプトを提示する。「引き継ぎお願いします」「このセッションを閉じたい」「memory に残して」のときに使う。複数セッションへ分割するなら session-dispatch。
...
- 担当表・振り分け表（`session-dispatch` が作るもの）: 自分の担当の行を「完了 / 保留」に直し、引き継ぎ memory へのリンクを足す
...
| セッション内で作った比喩を残す | 次のセッションがそれを正式な用語として使い、ユーザーに伝わらなくなる | `~/.claude/docs/glossary/jargon.md` の標準語に置き換えてから書く |
