...
他のレビュアーが既に出している指摘と、peer-review の指摘を**重複させない**。同じ論点を 2 人が別々に書くと、作成者の返信先が分散する。

### 重複指摘への対応

- **既存指摘に賛同する場合**: トップコメントで軽く触れる（「〇〇さんの指摘の通り、〜については対応をお願いします」）
- **既存指摘に反論する場合**: トップコメントで明示的に「〇〇さんの指摘とは異なる意見ですが、〜」
- **既存指摘の対応方針を促す場合**: 「〇〇さんの指摘（line X）への対応方針を PR 本文かコメントで明示していただけると助かります」

### 既存指摘を peer-review でも取り上げる判断基準

- 既存指摘が **設計の核心** に関わる場合は peer-review でも取り上げる
- 既存指摘が **対応されないまま approve されている** 場合、対応方針確定を促す
