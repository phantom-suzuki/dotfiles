...
fork からの PR は head ブランチが `origin` に存在しないため、`origin/<head ブランチ>` 形式では解決できない。**base・head とも PR のメタデータから値を取得**し、head は `headRefOid`（コミット SHA）で worktree を作る。

```bash
git fetch origin --quiet
SCRATCH="<scratchpad ディレクトリ>"   # セッションの一時領域
BASE_REF=$(gh pr view <N> --repo <owner>/<repo> --json baseRefName --jq '.baseRefName')
...
- `BASE_REF` は次項の `codex:codex-rescue` プロンプトに渡す base ブランチ名（`origin/main` 固定ではなく、PR ごとの実際の base を使う）。
...
- **文脈**: 「この worktree は PR #<N> のブランチ。base は `$BASE_REF`（手順 1 で取得した実際の base ブランチ）」
...
このディレクトリは GitHub PR #<N> のブランチをチェックアウトした git worktree です（<owner>/<repo>）。base は origin/$BASE_REF です（手順 1 で取得した実際の base ブランチ。origin/main 固定ではない）。
