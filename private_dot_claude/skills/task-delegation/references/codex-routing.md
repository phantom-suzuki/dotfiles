...
2 番目の条件は、**成果物の形を変えない範囲の不確かさ**だけを指します。要件や設計そのものが決まっていない依頼は、そもそも Codex へ委譲しません。統括セッション（T1）が対話で詰めます（`SKILL.md` の「T1 が正当化されるケース」）。
...
| `gpt-5.6-sol` × `high` | タスクを分割するか、T1 統括セッションへ戻す |
| `gpt-5.6-sol` × `xhigh` | T1 統括セッションへ戻す |
