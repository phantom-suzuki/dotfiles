...
description: 実装・修正・レビュー対応・文章化を委譲するかどうかと委譲先（Codex / サブエージェント）を判断する基準。委譲を検討するときに使う。
---

# Task Delegation Skill

実装タスクを受領したとき、Tier 判定を強制実行し、委譲先を決定する。統括セッション（セッションのメインモデル）のセッション温存とレートリミット対策のため、コード関連作業はデフォルトで実行役へ委譲する。判定をスキップして Edit/Write/Bash 等を呼ぶのは禁止。
...
委譲マトリクスは**役割ベース**で定義する。各役割を「どのモデル / どのサブエージェントが担うか」は、環境（セッションのモデル構成・Codex プラグインの導入有無）によって変わる。統括セッションのモデルは可変（セッションのメインモデルが担う）という前提に立つ。

| 役割 | 実体 |
|------|------|
| T1 統括セッション | セッションのメインモデル |
...
- **Codex 導入済み環境**: コード関連作業の既定は実行サブエージェント（T3）へ委譲する。Codex（T2）を選ぶのは下の「デフォルト方針」に挙げた 3 条件に当てはまるときだけである。検索・要約等の軽作業は軽い実行役（T3）へ委譲する。
- **Codex 未導入環境**: **T2 を skip** し、T2 相当のコード関連作業も含めてすべて実行サブエージェント（T3）へ委譲する。難度に応じて軽い実行役（定型・大量出力）と重い実行役（中難度・難所）を使い分ける。実行作業に汎用サブエージェントは使わない（高価な統括セッションモデルを継承するため）。
...
- 統括セッションが直接実装するのは**対話を伴う場合のみ**（要件確認、ADR 議論中の探索、複数案比較）
- diff 検証とコミット判断は統括セッション（T1）に残す（git-safety.md 継続）
- 委譲オーバーヘッドが大きく見えても、統括セッションの温存の方が長期的に得
...
- 委譲先: T1 統括セッション / T2 外部 CLI（Codex）/ T3 実行サブエージェント（軽い実行役 / 重い実行役）
...
> カラールール: 🔵 統括セッションが担当 / 🟡 Codex へ委譲（枠が有限） / 🟢 実行サブエージェントへ委譲（既定）

```mermaid
graph TD
  start[委譲タスクを受け取る] --> dialog{対話を伴うか<br/>要件確認・設計議論・複数案比較}
  dialog -- はい --> t1[T1 統括セッションが自分で実装する]
...
| Tier | 役割 | 対象タスク |
|------|------|----------|
| T1 | 統括セッション（セッションのメインモデル） | 設計判断・横断分析・対話・複数案比較・diff 検証・コミット判断 |
...
### T1（統括セッションが直接）が正当化されるケース

以下に該当する場合のみ統括セッションが直接コードを書いてよい:
...
上記以外で統括セッションが直接実装に入るのは**アンチパターン**。
...
- テストコード作成（テスト対象と方針は統括セッションが指定）
- メカニカルリファクタ（リネーム、import 整理、コールサイト更新）
- **レビュー指摘対応**（レビューコメントへの修正）
...
の優位が分かれる。標準環境で T1 統括セッションが自分でやるか T2 Codex へ回すか迷ったら、
...
| ComputerUse（PC 操作） | Claude | 統括セッション側で対応 |
| ブラウザエージェント操作 | Claude | 統括セッション側で対応 |
| 図表・ビジョン読解 | Claude | 統括セッション側で対応 |
| 対話・要件詰め・設計議論・ADR | Claude | T1 統括セッション（対話を伴うため） |

- **Codex 優位の領域**（自律実行 / Web リサーチ / 音声 / 画像生成）は、コード関連作業の
  default 委譲に加えて積極的に T2 へ回す。
- **Claude 優位の領域**（GUI 操作 / 図表読解 / 対話設計）は委譲せず統括セッション側で扱う。
- **Codex 未導入環境**では Codex 優位領域も委譲先が無いため、T3 実行サブエージェントで
  代替するか統括セッションが扱う。ただし画像生成（gpt-image 系）など Claude 側に代替手段が無い
  領域は代替できない旨をユーザーに伝える。

Codex 側の `~/.codex/AGENTS.md`（グローバル指針）には役割・禁止事項・スコープ・報告形式だけを短く書いてある。得意領域の一覧は載せない（この表が正本）。
...
## 完了条件
（必ず書く。何ができたら完了か、何を確認したら止まってよいかを具体的に書く。例: テストが通る、lint が通る、grep で該当 0 件、差分の要約を報告する。GPT-6 Astra は完了条件が無いと途中で止まる、または必要以上に続けることがある）
```

完了条件が無い委譲は禁止する。Codex は完了条件を満たしたら止まり、満たすまでは続ける（`~/.codex/AGENTS.md` のタスクスコープ）。統括セッションは完了条件を「検証できる形」で書く。
...
| 4 回目以降 | Codex をやめる。タスクを分割し直して T3 実行サブエージェントへ回すか、統括セッションが引き取る |
...
Codex 未導入環境で T2 相当タスクを実行サブエージェントへ委譲する場合、Codex 委譲と同じ構造（タスク / コンテキスト / 制約 / 完了条件）でプロンプトを組む。制約セクションに「git commit や git push はしないこと」を明記し、diff 検証とコミット判断は統括セッションに残す。

## 安全制約

- **実行役にコミット・プッシュさせない**。Codex 委譲でも実行サブエージェント委譲でも、プロンプト末尾に「git commit や git push は実行しないこと」と明記
- 結果は必ず統括セッション（T1）が `git diff` で検証してから受け入れる
...
> 「実装タスクは委譲対象なので、実行サブエージェント（T3）に委譲して実装させます」（既定。定型・大量出力なら軽い実行役、中難度・難所なら重い実行役と、選んだ側を明示する）
> 「本タスクは自律的な試行錯誤を伴うため、Codex に委譲します（T2 の根拠: Codex 優位領域）」
> 「本タスクは設計判断を含むため統括セッション（T1）で自分が実装します（理由: ...）」

## リポジトリ衛生チェック（commit 前 / 完了報告前 必須）

実装作業中に発生した artifact / 大型ファイルが commit に混入しないよう、必ず確認する。実行役（Codex / 実行サブエージェント）へ委譲して実装した場合も、統括セッションが diff 検証時に併せて行う:
...
2. それでも改善しない場合、より強い実行役へリルート（軽い実行役 → 重い実行役 → 統括セッション（T1））
3. 規模が大きい場合はタスク分割
...
- **「単発 Issue だから並列化不要 → 統括セッションが直接」** ← 並列化要否と委譲要否は独立軸。単発でも委譲対象なら実行役へ
- **「設計読み込み完了 → 自分で書く」** ← 設計が固まったなら次は実行役に渡すフェーズ
- **「400 行くらいなら自分で書いた方が速い」** ← 統括セッションのトークン浪費の典型。規模が大きいほど委譲効果が大きい
- **「軽微修正だから統括セッションが直接」** ← 10 行ルールは撤廃済。対話を伴わない限り委譲
...
- **「同じタスクに 4 回目以降も Codex を投げ直す」** ← 3 回で終わらないのはタスクの切り方の問題。分割し直して T3 か統括セッションへ回す
- **「上限が近いのに実測せず走り続ける」** ← 「レート予算の規律」節の実測コマンドで消費を確認してから判断する
- **「Codex 未導入環境で実行作業に汎用サブエージェントを使う」** ← 高価な統括セッションモデルを継承する。必ず軽い実行役 / 重い実行役の専用サブエージェントを使う
...
- v1（初版）: 10 行未満ルール / レビュー対応は統括セッション直接が暗黙
...
- v4（2026-07-06）: 委譲マトリクスを役割ベース（T1 統括セッション / T2 外部 CLI 委譲 = Codex / T3 実行サブエージェント）に一般化。統括セッションのモデルは可変（セッションのメインモデル）とし、Codex 未導入環境では T2 を skip し T3 へフォールバックする分岐を明記。正本をスキルへ統合（Issue #24）
...
- v7（2026-09-04）: T2 の委譲時にモデルと effort を毎回明示するルールと、選び方の正本 `references/codex-routing.md` を追加
- v8（2026-09-08）: GPT-6 Astra 向けに完了条件を必須化。Codex の `~/.codex/AGENTS.md` を目次サイズに縮めた（dotfiles PR）
