...
| `--simplify-via` | `internal` | `internal`（Opus 自身の `/simplify`）/ `codex`（`codex:codex-rescue` サブエージェント経由で委譲、レート消費に注意） |
...
3. 利用可能なレビュアーと環境を検出する:
   ...
   **Codex の可否判定は Bash から直接行わない**（`command -v codex` は PreToolUse フック
   ...
   呼び出し先に委ねる:
   - Step 1-2（Simplify 委譲）: `codex:codex-rescue` サブエージェントが可否判定とエラー報告を行う
...
Codex 委譲が失敗した場合（`codex:codex-rescue` が Codex 未導入・未認証と報告した場合を含む）は
...
| 観点 | primary | fallback (順) | プロンプト |
|-----|---------|--------------|----------|
| bug | `claude ultrareview --json` | `claude -p --bare ...` → Codex（`scripts/codex-review.sh`） | `review-prompt-bug.md` |
| security | Codex（`scripts/codex-review.sh`） | Claude-p → Gemini（`--with-gemini` 時） | `review-prompt-security.md` |
| design | `claude -p --bare ...`（`--with-gemini` 時は Gemini） | Codex（`scripts/codex-review.sh`） | `review-prompt-design.md` |
