...
- `--deep` 指定時に standard を superset：design 観点 + macro 観点（goal-achievement / spec-consistency）+ ultrareview を自動有効化し、5 観点並列 + `--max-iterations 3` まで拡張
- macro 観点は「diff だけでは分からないゴール達成度・Spec 整合」を見る。起動時に Step 0.5（コンテキスト収集）で PR / Issue / AC / ADR / CLAUDE.md を集めてからレビュアーに渡す
- 代替案（alternatives）は単独観点にせず、design と goal-achievement のプロンプトに「主要判断の代替案検討」として吸収する
...
| `--deep` | false | 重厚モード: `--strategy deep --max-iterations 3 --with-design --ultrareview` を自動適用し、macro 観点（goal-achievement / spec-consistency）を加えた 5 観点並列にする |
...
- 以下の CLI が利用可能であること（なければ自動スキップまたはフォールバック）:
  ...
  - `codex` (Codex CLI、外部レビュアー兼 `--simplify-via=codex` の委譲先。**Bash から `codex` を直接実行しない**。PreToolUse フック `block-codex-direct.py` が `codex` の直接実行をブロックするため、Simplify 委譲は `codex:codex-rescue` サブエージェント（対話なら `/codex:rescue`）、security/design レビューはスキル同梱の [scripts/codex-review.sh](scripts/codex-review.sh) 経由で呼び出す。詳細は Step 1-2 と Step 3 を参照)
...
### Step 0.5: macro 観点のコンテキスト収集（`deep` 時のみ）

macro 観点（goal-achievement / spec-consistency）は **diff だけでは評価できない**。目的達成度は「PR / Issue が何を達成しようとしたか」を、Spec 整合は「関連 ADR / Spec / 規約が何を決めているか」を前提に判定する。そのため deep モードでは Simplify の前にコンテキストを集めておく。

**このステップは `deep`（macro 観点が経路に入る場合）でのみ実行する**。standard / simple / `--with-design` 単独ではスキップする（micro 観点はコンテキスト不要）。

手順は peer-review の [context-collection.md](../peer-review/references/context-collection.md)（コンテキスト収集手順）の L0〜L2 を流用する。self-review は「自分の未マージ変更」を見るため、PR が未作成のことがある点だけ扱いが異なる。

1. **PR / Issue の抽出**
   - 現ブランチに PR があれば `gh pr view --json title,body,files` で本文を取る。無ければコミットメッセージ（`git log $BASE_BRANCH..HEAD`）とブランチ名を代わりに使う
   - 本文・コミットメッセージ・ブランチ名から `#\d+`（Issue / Epic 参照）を抽出し、`gh issue view <N> --json title,body` で本文と受け入れ基準（AC / DoD）を取得する
2. **ADR / Spec の抽出**
   - diff / PR 本文 / コミットメッセージで参照される `.md`（ADR / Spec / 設計資料）を Read する。ステータス欄（承認済み / 提案中）も確認する
3. **規約の収集**
   - 対象ディレクトリの `CLAUDE.md` と、リポジトリの `.claude/rules/*.md` のうち対象領域に関係するものを Read する
4. **コンテキストの束ね方**
   - 集めた内容を 1 つの文字列（AC 一覧 → 参照 ADR/Spec 要点 → 規約要点）にまとめ、一時ファイル（`$MACRO_CONTEXT_FILE`）に書き出す
   - この文字列は Step 3 で goal / spec のプロンプトの `<MACRO_CONTEXT>` に差し込まれる（[references/review-prompts.md](references/review-prompts.md) の `build_prompt` 参照）
   - 何も集まらなかった場合は空のまま進める。プロンプト側が「（PR / Issue コンテキストなし）」で埋め、推定に依存する指摘を info に格下げする

収集量が多い場合は要点だけ抜き出す（AC の各項目・ADR のステータスと結論・規約の禁止事項）。diff / 資料の全文をそのままプロンプトに流し込まない。
...
Agent ツールで `subagent_type: "codex:codex-rescue"` を起動し、以下のタスク文面をそのままプロンプトとして渡す
（書き込みを伴う作業のため、rescue 側の既定の書き込み可能モードに任せる）:
...
|-----|---------|--------------|----------|
| bug | `claude -p --bare ...`（`--ultrareview` 時は `claude ultrareview --json`） | Codex（`scripts/codex-review.sh`） → Gemini（`--with-gemini` 時のみ） | `review-prompt-bug.md` |
| security | Codex（`scripts/codex-review.sh`） | Claude-p（`--bare`） → Gemini（`--with-gemini` 時のみ） | `review-prompt-security.md` |
...
#### strategy: deep（重厚モード・5 並列 + ultrareview）

`--deep` 指定時、または `--strategy deep` 明示時。standard を superset し、design 観点 + macro 観点（goal-achievement / spec-consistency）を追加して 5 並列にする。macro 観点は Step 0.5 で集めた `$MACRO_CONTEXT_FILE` を `<MACRO_CONTEXT>` に差し込んでからレビュアーへ渡す。

下表は「役割（一次レビュアー / 代替）＋その役割に現在割り当てているモデル（現行 …）」の併記で書く。役割を主語にすることで、モデルが入れ替わっても表の意味が壊れない（Epic #21 の方針A: 非モデル依存 / 方針B: 精度を落とさずモデル固有値は保持）。

| 観点 | 一次レビュアー | 代替（順） | プロンプト |
|-----|--------------|----------|----------|
| bug | 現行 `claude ultrareview --json` | 現行 `claude -p --bare ...` → Codex（`scripts/codex-review.sh`） | `review-prompt-bug.md` |
| security | 現行 Codex（`scripts/codex-review.sh`） | 現行 Claude-p → Gemini（`--with-gemini` 時） | `review-prompt-security.md` |
| design | 現行 Claude-p（`--with-gemini` 時は Gemini） | 現行 Codex（`scripts/codex-review.sh`） | `review-prompt-design.md` |
| goal-achievement | 現行 Claude-p | 現行 Gemini（`--with-gemini` 時） → Codex（`scripts/codex-review.sh`） | `review-prompt-goal.md` |
| spec-consistency | 現行 Codex（`scripts/codex-review.sh`） | 現行 Claude-p → Gemini（`--with-gemini` 時） | `review-prompt-spec.md` |

`--with-gemini` が無い場合、Gemini 経路は完全に外す（`gemini-preflight.sh` も呼ばない）。

5 並列はリソース負荷が高いため、macro 観点（goal-achievement / spec-consistency）を回すのは `deep` に限定する。standard / `--with-design` では起動しない。
...
2. 各 finding の `aspect: bug|security|design|goal-achievement|spec-consistency` タグを保持する。
...
- 観点別件数（例: standard なら `bug: 3 / security: 1`、`--with-design` なら `bug: 3 / security: 1 / design: 2`、deep なら macro 観点を加えて `bug: 3 / security: 1 / design: 2 / goal-achievement: 1 / spec-consistency: 0`）
