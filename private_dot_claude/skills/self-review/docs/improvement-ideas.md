...
- 全 CLI 呼び出しに **再現性確保フラグを必須化**:
  ...
  - `codex exec`: `--output-schema <FINDING_SCHEMA> --ignore-user-config --ignore-rules`、モデルは `-c model=gpt-5.6-terra`（`gpt-5-codex` 系の schema 無視バグ回避。terra は schema 順守を確認済み。`gpt-5` は ChatGPT アカウント認証で 400 のため不可）
