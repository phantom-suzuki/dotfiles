...
**Bash から `codex exec` を直接叩かない**。PreToolUse フック `block-codex-direct.py` は Bash ツールに
渡すコマンド文字列の先頭トークンを検査してブロックするため、`codex exec ...` をそのまま Bash に渡すと
弾かれる。`codex exec` の呼び出しは [../scripts/codex-review.sh](../scripts/codex-review.sh) に閉じ込め、
Bash からは**スクリプトを呼ぶ**（スクリプトファイルの中身は同フックの検査対象外）。
同スクリプトが `codex exec review` の `--output-schema` 非対応問題を踏まえ、`exec`（review なし）+
プロンプト埋め込みで実装している。
...
`scripts/codex-review.sh` が起動時に以下の判定を行う（旧: Step 0 で `CODEX_REPRO_FLAGS` として組み立てていたロジックをスクリプトへ移設）:
...
`--ignore-user-config` / `--ignore-rules` は `codex >= 0.122` で導入。古い codex では未対応のため空にする。
...
スキーマ強制が要求される本スキルでは **`-c model=gpt-5.6-terra`**（CLI 明示指定。
`--ignore-user-config` 併用時も有効。terra は `--output-schema` 順守を 2026-07-14 に確認）を
使う。上書きは環境変数 `CODEX_REVIEW_MODEL`。
