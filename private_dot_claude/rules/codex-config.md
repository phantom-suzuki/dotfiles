...
Codex は AGENTS.md を 2 層で読み込み、近い方が後勝ちで上書き連結する。

| 層 | パス | 役割 |
|---|---|---|
| グローバル | `~/.codex/AGENTS.md`（`dot_codex/AGENTS.md`） | 全プロジェクト共通の委譲実行指針 |
| プロジェクト | 作業ディレクトリ直下の `AGENTS.md` | リポジトリ固有の規約 |

- 競合したら**プロジェクト側が優先**（近い方が後勝ち）。
- 読み込みサイズは `project_doc_max_bytes`（config.toml）で制御する。グローバル AGENTS.md は
  Codex の context 上限を意識して短く保ち、固有規約はプロジェクト直下に置く。
...
|---|---|---|
| CLAUDE.md | `AGENTS.md` | 2 層スコープ・近い方が後勝ち・`project_doc_max_bytes` で制御 |
