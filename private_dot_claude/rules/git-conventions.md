...
- **PR は必ず 1 つ以上の Issue を `Closes` で閉じる（Refs 運用は原則禁止）**: PR のマージで何かしらの Issue が必ずクローズされる状態にする。PR で AC を満たしきれない Issue に対しては、Refs で逃げずに **Issue を分割**する。Feature なら Task に詳細化して Sub-issues にし、PR はその Task を Closes する（残 AC は兄弟 Task が引き継ぐ）。実データ投入・実機確認などマージ後にしかできない AC も、同じ要領で別 Task に切り出す（ユーザー指示 2026-08-02。過去にも複数回同趣旨の指摘あり）
