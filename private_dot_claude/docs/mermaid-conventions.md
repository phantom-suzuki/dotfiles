...
| In Progress / 進行中 | `#9a6700` | `#7d4e00` | `#ffffff` | attention 系 |
...
graph LR
  ...
  classDef inprog fill:#9a6700,stroke:#7d4e00,color:#ffffff;
...
EOF
node check.mjs diagram.mmd   # 抽出した .mmd ファイルのパスを引数で渡す（process.argv[2] で読む）
```
